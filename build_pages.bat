...
echo --------------------------------------------------
echo Do you want to make milti page html for big files?
echo        
echo --------------------------------------------------
echo:
choice
set _pgop=n
if %errorlevel% equ 1 set _pgop=y
cls
...
REM _bin\xidel -s _temp\temp.1 -e "replace( $raw, '^(.+?)\t[a-z]+\t(\w+)', ' 		<a href=\""%_pre%main.html#$2\"" target=\""main\"">$1</a><br>', 'm')" >>_temp\%_pre%side.html
...
rem //variables to make multiple page html


if %_total_lines% lss 1000 set _pgmax=1
if %_total_lines% geq 1000 set _pgmax=2
if %_total_lines% geq 2000 set _pgmax=3
if %_total_lines% geq 3000 set _pgmax=4
if %_total_lines% geq 4000 set _pgmax=5
if %_total_lines% geq 5000 set _pgmax=6
if %_total_lines% geq 6000 set _pgmax=7
if %_total_lines% geq 7000 set _pgmax=8
if %_total_lines% geq 8000 set _pgmax=9
if %_total_lines% geq 9000 set _pgmax=10
if %_total_lines% geq 10000 set _pgmax=11


rem //overwrite from user input
if %_pgop%==n set _pgmax=1

set _pg=1
set /a _roms_div=%_total_lines%/%_pgmax%
set _roms_count=0


rem // build side title frame
for /f "tokens=1-3 delims=	" %%g in (_temp\parents.txt) do (

	set /a _roms_count+=1
	if !_roms_count! equ %_roms_div% (
		if !_pg! lss %_pgmax% (
			REM call :end_main !_pg!
			set /a _pg+=1
			set _roms_count=0
			REM call :start_main !_pg!
		)
		
	)
	title !_roms_count!
	echo 	^<a href="%_pre%main_!_pg!.html#%%g" target="main"^>%%i^</a^>^<br^>
	
) >>_temp\%_pre%side.html


set _pg=1
set _roms_count=0

rem //zero wont have an end?
call :start_main !_pg!

REM cls&echo Building main.html....
...
for /f "tokens=1-3 delims=	" %%g in (_temp\parents.txt) do (
	...
	title "%_dat%" !_count_lines! / !_total_lines! ^( !_percent! %% ^) %_pgmax%
	...
	if !_roms_count! equ %_roms_div% (
		if !_pg! lss %_pgmax% (
			...
			REM echo 		^<a href="%_pre%main_!_pg!.html" target="_blank"^>^</a^>^<br^>
			echo ^<center^>^<strong^>
			for /l %%k in (1,1,%_pgmax%) do echo 		^<a href="%_pre%main_%%k.html" target="main"^>^&emsp;Page %%k^</a^>
			echo ^</center^>^</strong^>
			echo ^</body^>
			echo ^</html^>
...
) >>_temp\%_pre%main_!_pg!.html


rem //end the last page here

(echo ^<center^>^<strong^>
for /l %%k in (1,1,%_pgmax%) do echo 		^<a href="%_pre%main_%%k.html" target="main"^>^&emsp;Page %%k^</a^>
echo ^</center^>^</strong^>
echo ^</body^>
echo ^</html^>) >>_temp\%_pre%main_!_pg!.html
...
REM cls&title making copy image script...
...
REM copy /y _temp\%_pre%main.html "output\%_dat:~0,-4%\visual_database"
for /l %%k in (1,1,%_pgmax%) do copy /y _temp\%_pre%main_%%k.html "output\%_dat:~0,-4%\visual_database"
...
REM (echo ^</body^>
REM echo ^</html^>) >>_temp\%_pre%main.html
...
REM (echo ^</body^>
REM echo ^</html^>) >>_temp\%_pre%main_%1.html

(echo ^<^^!DOCTYPE html^>
echo ^<html^>
echo ^<head^>
for %%g in ("%_dat:~0,-4%") do echo 	^<title^>%%~g Page %1^</title^>
echo 	^<style^>
...
echo ^<center^>^<strong^>
for /l %%k in (1,1,%_pgmax%) do echo 		^<a href="%_pre%main_%%k.html" target="main"^>^&emsp;Page %%k^</a^>
echo ^</center^>^</strong^>) >_temp\%_pre%main_%1.html
...
(echo ^</body^>
echo ^</html^>) >>_temp\%_pre%main_%1.html
...
echo 	^<frame name="main" src="visual_database\%_pre%main_1.html"/^>
echo 	^<noframes^>
...
REM echo ^<html^>
REM echo ^<head^>
REM for %%g in ("%_dat:~0,-4%") do echo 	^<title^>%%~g^</title^>
REM echo 	^<style^>
REM echo 		body { background-color:powderblue; }
REM echo 	^</style^>
REM echo ^</head^>
REM echo ^<body^>) >_temp\%_pre%main.html
